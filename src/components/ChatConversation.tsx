...
export default function ChatConversation(props: ChatConversationProps) {
  ...
  const loadConversationStarters = async () => {
    ...
    try {
      ...
      const response = await props.chatSuggestionService.getConversationStarters(
  ...
  const requestSuggestion = async (text: string) => {
    ...
    try {
      const chatContext = props.chat ? props.chatSuggestionService.buildChatContext(props.chat.messages, 3) : '';
  ...
  const openSideChat = () => {
    ...
    const context = props.chat ? props.chatSuggestionService.buildChatContext(props.chat.messages, 2) : '';
  ...
  const requestTranslation = async (selectedText: string, messageContent: string) => {
    ...
    try {
      ...
      const response = await props.chatSuggestionService.getTranslation({
        selectedText,
  ...
  const openTranslationSideChat = () => {
    ...
    const context = props.chatSuggestionService.buildChatContext(props.chat.messages, 2);
  ...
  createEffect(() => {
    if (props.chat && props.chat.messages.length === 0 && assistantQuestions().length === 0 && userQuestions().length === 0 && !isLoadingStarters()) {
